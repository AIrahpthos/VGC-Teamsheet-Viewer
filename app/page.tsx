...
import type { PdfRoster } from "@/lib/pdf-teams";
import { storedPdf } from "@/lib/pdf-storage";
import { Layers, Search } from "lucide-react";
import { Combobox, ComboboxInput, ComboboxContent, ComboboxList, ComboboxItem, ComboboxEmpty } from "@/components/ui/combobox";
import { Skeleton } from "@/components/ui/skeleton";
import { Empty, EmptyHeader, EmptyTitle, EmptyDescription } from "@/components/ui/empty";
type Tournament={id:string;name:string};
type Player={name:string;division:string;country:string;team:string|null;pdfPage?:number;trainer?:string;pokemon?:Pokemon[];issue?:string};
...
export default function Home(){
 ...
 const [pdf,setPdf]=useState<PdfRoster|null>(null),[importProgress,setImportProgress]=useState(''),[pdfNotice,setPdfNotice]=useState('');
 const fileInput=useRef<HTMLInputElement>(null);
 const isPdf=tournament?.id==='local-pdf';
 async function loadTournaments(){const seq=sequence.current;setLoading('tournaments');setError('');try{const d=await api({action:'tournaments'});setTournaments(d.tournaments)}catch(e){if(seq===sequence.current)setError((e as Error).message)}finally{setLoading(current=>current==='tournaments'?'':current)}}
 useEffect(()=>{loadTournaments();const seq=sequence.current;void storedPdf().then(saved=>{if(saved?.version===1 && sequence.current===seq){setPdf(saved);sequence.current++;setTournament({id:'local-pdf',name:saved.name+' (PDF)'});setPlayers(saved.players);setLoading('');setError('')}}).catch(()=>{});},[]);
 async function chooseTournament(t:Tournament|null){const seq=++sequence.current;setTournament(t);setSelected(null);setTeam([]);setPlayers([]);setQuery('');setError('');setLoading('');if(!t)return;if(t.id==='local-pdf'){setPlayers(pdf?.players||[]);return;}setLoading('roster');try{const d=await api({action:'roster',id:t.id});if(seq===sequence.current){setPlayers(d.players);if(d.name)setTournament({id:t.id,name:d.name})}}catch(e){if(seq===sequence.current)setError((e as Error).message)}finally{if(seq===sequence.current)setLoading('')}}
 async function choosePlayer(p:Player){const seq=++sequence.current;setSelected(p);setTeam([]);setError('');setLoading('');if(p.pdfPage){setTeam(p.pokemon||[]);return;}if(!p.team)return;setLoading('team');try{const d=await api({action:'team',path:p.team});if(seq===sequence.current)setTeam(d.team)}catch(e){if(seq===sequence.current)setError((e as Error).message)}finally{if(seq===sequence.current)setLoading('')}}
 async function readPdf(file:File){const seq=++sequence.current;setLoading('');setError('');setImportProgress('Opening PDF…');setPdfNotice('');try{const {importPdf}=await import('@/lib/import-pdf');const imported=await importPdf(file,(page,total)=>setImportProgress(`Reading PDF: ${page} / ${total}`));if(seq!==sequence.current)return;setPdf(imported);setTournament({id:'local-pdf',name:imported.name+' (PDF)'});setPlayers(imported.players);setSelected(null);setTeam([]);setQuery('');try{await storedPdf(imported)}catch{setPdfNotice('Imported successfully, but this browser could not save it. Import again after refreshing.')}}catch(e){if(seq===sequence.current)setPdfNotice(e instanceof Error?e.message:'The PDF could not be imported. Try an unencrypted RK9 tournament PDF.')}finally{setImportProgress('');if(fileInput.current)fileInput.current.value=''}}
 async function removePdf(){try{await storedPdf(null);setPdf(null);setPdfNotice('');if(isPdf)await chooseTournament(null)}catch{setPdfNotice('The saved PDF could not be removed. Please try again.')}}
 ...
 {!loading&&!tournament&&<Empty className="empty-state"><EmptyHeader><Search size={28} className="mx-auto mb-3 text-blue-600"/><EmptyTitle>Find your opponent’s team</EmptyTitle><EmptyDescription>Choose an RK9 tournament or import a teamsheet PDF, then search for a player.</EmptyDescription></EmptyHeader></Empty>}
 {!loading&&tournament&&!selected&&<section><div className="section-head"><h2>{query?'Players':'Find a player'}</h2><span className="muted">{players.length.toLocaleString()} registered</span></div>{!query?<p className="muted">Enter a name above to search this tournament’s roster.</p>:matches.length?<div className="players">{matches.slice(0,50).map((p,i)=><button disabled={!!importProgress} className="player" key={p.name+p.team+i} onClick={()=>choosePlayer(p)}><span><strong>{p.name}</strong><br/><small>{[p.division,p.country,p.pdfPage?`PDF page ${p.pdfPage}`:''].filter(Boolean).join(' · ')}</small></span><small>{p.team?'View team →':p.pdfPage?'Blank / unreadable':'Not published'}</small></button>)}{matches.length>50&&<p className="muted">Keep typing to narrow down {matches.length} matches.</p>}</div>:<p className="muted">No players match “{query}”. Try a first or last name.</p>}</section>}
 {selected&&<section><div className="section-head"><h2>{selected.name}</h2><span className="muted">{selected.division}</span>{selected.pdfPage&&<span className="muted">PDF page {selected.pdfPage}</span>}{selected.team&&!selected.pdfPage&&<a href={'https://rk9.gg'+selected.team} target="_blank" rel="noreferrer">Original on RK9 ↗</a>}</div>{!selected.team?<div className="empty-state"><h2>{selected.pdfPage?'Teamsheet unavailable':'Teamsheet not published'}</h2><p>{selected.issue||'RK9 does not currently provide a public teamsheet for this player.'}</p></div>:<div className="team">{team.map((p,i)=><article className="pokemon" key={i}><header className="pokemon-head"><PokemonIcon key={p.name + (p.sprite || "")} name={p.name} fallback={p.sprite} slot={i+1}/><div><h3>{p.name}</h3>{p.tera&&<span className="tera">Tera · {p.tera}</span>}</div></header><dl className="details"><div><dt>Ability</dt><dd>{p.ability||'—'}</dd></div><div><dt>Held item</dt><dd>{heldItemLabel(p.item)}</dd></div></dl><ul className="moves">{p.moves.map((m,j)=><li key={j}>{m||'—'}</li>)}</ul>{p.extra&&<p className="muted px-5 pb-4">{p.extra}</p>}</article>)}</div>}</section>}<footer className="footer">RK9 teamsheets and imported PDFs. Unofficial viewer; not affiliated with RK9 or Pokémon.</footer></main>
}
